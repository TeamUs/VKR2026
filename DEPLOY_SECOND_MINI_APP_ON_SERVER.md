...
### 3.2 Вариант B: та же база, другая схема/префикс таблиц

Если решите использовать одну базу с разными таблицами (например, префикс `vkr_`) — это нужно предусмотреть в коде и в схеме. Обычно проще отдельная база (вариант A).
